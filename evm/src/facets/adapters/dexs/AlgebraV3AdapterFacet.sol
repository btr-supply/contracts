...
contract AlgebraV3AdapterFacet is V3AdapterFacet {
    ...
    {
        (tickCumulatives, intervalSecondsX128,,) = IAlgebraV3Pool(pool).getTimepoints(secondsAgos);
    ...
    {
        (liquidity,,,, fees0, fees1) = IQuickV3Pool(pool).positions(positionId);
    }
    ...
    {
        (amount0, amount1,) =
            IQuickV3Pool(pool).mint(address(this), address(this), tickLower, tickUpper, liquidity, abi.encode(pool));
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (amount0, amount1) = IQuickV3Pool(pool).burn(tickLower, tickUpper, liquidity);
    }
