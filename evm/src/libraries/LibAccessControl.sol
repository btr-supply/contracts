...
library LibAccessControl {
    ...
    function accessControlStorage() internal view returns (AccessControl storage) {
    ...
    /**
     * @notice Enforce that the caller has the MANAGER_ROLE for a specific vault
     * @dev The vaultId parameter is currently unused but may be used for vault-specific permission checks in the future
     */
    function enforceIsManager(uint32 /* vaultId */) internal view {
        if (!hasRole(MANAGER_ROLE, msg.sender)) {
            revert Errors.Unauthorized(ErrorType.ROLE);
        }
        
        // Additional vault-specific checks could be added here if needed
    }
}
