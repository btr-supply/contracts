...
library AsCast {
  function toInt256(uint256 value) internal pure returns (int256) {
    ...
    if (value > uint256(type(int256).max)) revert Errors.ValueOutOfCastRange();
    return int256(value);
  }

  function toUint256(int256 value) internal pure returns (uint256) {
    // If value < 0, it will revert
    if (value < 0) revert Errors.ValueOutOfCastRange();
...
library Maths {
  ...
  uint256 internal constant BP_BASIS = 1000_0;
  ...
  function precisionBp(uint256 amount, uint256 basisPoints) internal pure returns (uint256) {
    unchecked {
